...
class TestAccessRight(TransactionCase):
    """Tests for permissions to users to 'product pricelist type' field."""
    def setUp(self):
        super(TestAccessRight, self).setUp()
        User = self.env['res.users']
        main_company = self.env.ref('base.main_company')
        res_user_group = self.env.ref('base.group_user')
        self.pricelist_type1 = self.env.ref('product.pricelist_type_sale')
    ...
    def test_user_access_rights_to_pricelist_type(self):
        ...
        """
        self.pricelist_type1.sudo(self.base_user).read()
